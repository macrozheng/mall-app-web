...
import { fetchReadHistoryListAPI, clearReadHistoryAPI } from '@/apis/memberReadHistory'
...
// 浏览记录列表
const productList = ref<MemberReadHistory[]>([])
// 分页请求参数
...
const loadData = async (type: 'refresh' | 'add' = 'add') => {
  ...
  try {
    const res = await fetchReadHistoryListAPI(searchParam.value)
...
// ===== 事件处理方法 =====
// 跳转商品详情页
const handleNavToDetail = (item: MemberReadHistory) => {
